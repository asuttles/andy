...
- Support extensions (floats, strings, richer functions).
...
* Common Lisp Intermediate Representation
  - Stage: Lowered representation of AST for code generation
  - Purpose: Simplify AST into explicit, CL-friendly constructs

** Program
   - (:program Type Consts Vars Funcs Body)
...
** Built-in Operations / Statements
   - Print: (:print expr)
   - Standard math: +, -, *, / (see :binop)

** Notes
   - Arrays and advanced types can be added as:
     - (:array-new type size)
     - (:array-ref arr idx)
